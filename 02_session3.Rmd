...
```{r, echo=FALSE, fig.cap="Leiden annotation-based spatial niches (top), stacked barplot of leiden annotation composition by niche (bottom)"}
...
```{r, echo=FALSE, fig.cap="Spatial plot of enriched leiden annotation interactions. 6 to 8 (top), 2 to 9 (bottom)"}
...
Which features are differentially expressed due to proximity to other cell types?

```{r, eval=FALSE}
# uses a lot of memory
icf <- findICF(g,
    cluster_column = "leiden_clus",
    selected_feats = featIDs(g),
    nr_permutations = 1000,
    adjust_method = "bonferroni"
)

scran_res <- findScranMarkers(g,
...
top_scran <- scran_res[, head(.SD, 2), by = "cluster"]

violinPlot(g,
    expression_values = "normalized",
    cluster_column = "leiden_clus",
    feats = unique(top_scran$feats)
)
