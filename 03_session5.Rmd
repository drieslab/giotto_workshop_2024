...
### settings
```{r, eval=FALSE}
options(timeout=Inf) # In case of network interreupt
data_path <- file.path("data","merfish")
dir.create(data_path)
results_folder <- file.path("results","merfish")
...
Note 2: due to the instability of network, download processing may be interrupt.
    We recommend to download these data in advance or download the processing giotto obj from [Zenodo](https://zenodo.org/records/13216587/files/merfish.zip?download=1).
```{r, eval = FALSE}
...
giotto_all_slices_obj <- anndataToGiotto("data/merfish/counts.h5ad")
...
spatial_locs_df <- data.frame(cell_ID = selected_cells,sdimx = centr.x,sdimy = centr.y)
...
giotto_obj = joinGiottoObjects(gobject_list = list(giotto_slice1_obj, giotto_slice2_obj),
                              gobject_names = c('mouse2_slice229',
                                                'mouse2_slice300'),  # name for each samples
                              join_method = 'z_stack')

## save giotto obj
saveRDS(giotto_obj, file = file.path(results_folder, "giotto_merfish_sbuset.Rds"))
```


If you facing network issue when downloading the raw dataset. Please download the processing giotto obj from [Zenodo](https://zenodo.org/records/13216587/files/merfish.zip?download=1), unzip and move it to `results` folder.s
```{r, eval=FALSE}
giotto_obj <- readRDS(file = file.path(results_folder, "giotto_merfish_sbuset.Rds"))
```
...
mkdir -p output/ONTrac
ONTraC -d data/merfish/ONTraC_dataset_input.csv --preprocessing-dir results/merfish/preprocessing_dir --GNN-dir results/merfish/GNN_dir --NTScore-dir results/merfish/NTScore_dir --device cuda     --epochs 1000 -s 42 --patience 100 --min-delta 0.001 --min-epochs 50 --lr 0.03     --hidden-feats 4 -k 6 --modularity-loss-weight 0.3 --purity-loss-weight 300 --regularization-loss-weight 0.3 --beta 0.03 2>&1 | tee results/merfish/merfish_subset.log
```
